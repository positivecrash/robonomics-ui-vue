<template>
  <template v-if="!rwsEndDate">
    <robo-text weight="bold" galign="center">No subscription found in <i class="networkname">{{network}}</i></robo-text>
  </template>
  <template v-else>
     <robo-grid v-if="narrowscreen" gap="x025" type="flex" galign="start">
        <robo-icon icon="user" />
    ...
    <robo-grid type="flex" gap="x1" class="robo-rws-devices-dashboard">
      <robo-grid type="flex" galign="start" gap="x1">
        <div>
...
import {downloadJson, shortenAddress} from '../tools'
