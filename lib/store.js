        ...
        rws: {
            ...
            actions: {
                ...
                rewrite({dispatch}, value) {
                    console.log('rewrite value', value)
                ...
                setUser({state, dispatch}, {rwsowner, value}) {
                    dispatch('findrws', rwsowner).then( index => {
                        if(index > -1) {
                            state.list[index].user = value
                            dispatch('rewrite', state.list)
                        }  
                    })
                },

                deleteUser({state, dispatch}, {rwsowner}) {
                    dispatch('findrws', rwsowner).then( index => {
                        if(index > -1) {
                            state.list[index].user = null
                            dispatch('rewrite', state.list)
                        }  
                    })
                }
