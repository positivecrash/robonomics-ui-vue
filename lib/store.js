            ...
            actions: {
                import({state, commit}, value) {
                    ...
                    if(value.length > 0 && typeof value === 'string') {
                        ...
                        localStorage.setItem('robonomicsUIrwslist', value)
                ...
                addUser({state, dispatch}, {rws, user}) {
                    const addto = state.list.map(item => item.owner).indexOf(rws.value)
                ...
                userexistance({state}, {rws, user}) {
                    ...
                    // const findrws = state.list.map(item => item.owner).indexOf(rws)
                    // return state.list[findrws].users.map(item => item.address).indexOf(user)
                },
