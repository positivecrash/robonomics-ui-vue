            ...
            actions: {
                ...
                editSetup({ state, commit, rootState }, v) {
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        if (index > -1) {
                            ...
                            commit('setSetupsList', state.list);
                            console.log('editSetup', state.list);
                            localStorage.setItem('robonomicsUIrwslist', JSON.stringify(state.list));
                ...
                uploadSetup({ dispatch, rootState }, value) {
                    if (value.length > 0 && typeof value === 'string') {
                        ...
                        // Получаем текущую сеть из хранилища
                        const currentNetwork = rootState.robonomicsUIvue.polkadot.connection.network;
                
                        if (importdata) {
                            const processSetup = (setup) => {
                                if (setup.network) {
                                    // Проверяем, совпадает ли указанная в setup сеть с текущей
                                    if (setup.network !== currentNetwork) {
                                        return { success: false, error: `Network mismatch! Imported setup is for '${setup.network}', but the current network is '${currentNetwork}'. Please switch networks and try again.` };
                                    }
                                } else {
                                    // Если в setup сеть не указана, устанавливаем текущую
                            ...
                            if (Array.isArray(importdata) && importdata.length > 0) {
                                // Если импортируется массив setup'ов, обрабатываем каждый
                                const results = importdata.map(processSetup);
                                // Проверяем, была ли ошибка в каком-либо setup
                                const hasError = results.find(result => !result.success);
                                return hasError || { success: true };
                            } else if (typeof importdata === 'object') {
                                // Если импортируется один setup, обрабатываем его
                                return processSetup(importdata);
                            }
                        }
                    }
                    // Если входные данные некорректные, возвращаем ошибку
