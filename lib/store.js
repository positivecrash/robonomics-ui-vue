    ...
    modules: {
        polkadot: {
            ...
            state: () => ({
                ...
                connection: {
                    network: '', /* polkadot, kusama — empty until detected to avoid undefined in robo-icon */
            ...
            mutations: {
                ...
                setGeneratedAccount(state, value) {
                    state.generatedAddress = value
                },
        ...
        rws: {
            ...
            state: () => ({
                ...
                devices: {
                    view: localStorage.getItem('devicesView') ?? 'category' // "category" или "area"
                },
                launch: null,
                user: {
            ...
            actions: {
                ...
                addSetup({ state, commit, dispatch, rootState }, v) {
                    const currentNetwork = rootState.robonomicsUIvue?.polkadot?.connection?.network || 'polkadot';
        ...
        ipfs: {
            ...
            state: () => ({
                availableGateways: (() => {
                    // надо ещё дефолтные добавить
                    const key = 'ipfs-smart-gateway:user-gateways';
                    try {
                ...
                activeGateway: localStorage.getItem('ipfs-smart-gateway:picked') ?? null,
            }),

            mutations: {
                setActiveGateway(state, url) {
                    state.activeGateway = url;
                },
                setGateways(state, urls) {
                    state.availableGateways = urls;
                },
            },

            actions: {
                updateGateways({ commit, state }, urls) {
                    commit('setGateways', urls);

                    if (urls.length > 0) {
                        if (!state.activeGateway || !urls.includes(state.activeGateway)) {
                            commit('setActiveGateway', urls[0]);
                        }
                    }
                }
            }
